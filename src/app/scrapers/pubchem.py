...
PUBCHEM_REST = settings.PUBCHEM_REST_URL
PUBCHEM_VIEW = settings.PUBCHEM_VIEW_URL

def _get_json(url: str, delay: float = 0.2) -> dict:
    time.sleep(delay if delay is not None else settings.SCRAPER_DELAY_SECONDS)
    req = urllib.request.Request(url, headers={'User-Agent': settings.PUBCHEM_USER_AGENT})
    try:
        with urllib.request.urlopen(req, timeout=settings.PUBCHEM_TIMEOUT_SECONDS) as response:
...
def fetch_epi_suite_data(smiles: str) -> dict:
    ...
    """
    url = f"{settings.EPI_SUITE_API_URL}?smiles={urllib.parse.quote(smiles)}"
    req = urllib.request.Request(url, headers={'User-Agent': settings.PUBCHEM_USER_AGENT})
    try:
        with urllib.request.urlopen(req, timeout=settings.PUBCHEM_TIMEOUT_SECONDS) as response:
